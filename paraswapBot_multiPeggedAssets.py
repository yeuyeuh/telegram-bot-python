...
def get_price():
    try:
        ...
        for index, row in swap_df.iterrows():
            ...
            response = requests.get(row['api_url'])
            data = response.json()
            ...
            if destAmount > row['threshold'] :
                #print(row['updateTimer'])
                if row['updateTimer']==0 :
                    message = f'Maintenant si tu swap {srcAmount:.2f} {row["srcToken_name"]}, tu obtiens {destAmount:.2f} {row["destToken_name"]} sur paraswap (avec un gas cost de ${gasCostUSD:.2f})\n{row["url"]}'
